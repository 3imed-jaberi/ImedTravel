  ...
  function fetchAllTrips($table){
      ...
      if (mysqli_num_rows($result) > 0) {
          ...
          while($row = mysqli_fetch_assoc($result)) {
              $trip = ['id' => $row["id"] , 'name' => $row['name'] ,  'description' => $row['description'] ,  'prix' => $row['prix'] , 'path' => $row['path']] ;
...
*/
  ...
  function sendEmail($admin_email , $mail , $msg){
          ...
          echo json_encode(['sended' => true , 'details' => 'if this functions don\'t return any result .. you should be put the preoject in production mode with servers ..']) ;
  }
...
*/
  ...
  function saveEmailInformations($table ,$data){
          ...
          if ($sql_response) {
              echo json_encode(['added' => true ,'res' => $sql_response]) ;
          } else {
              echo json_encode(['added' => false,'res' => $sql_response]) ;
          }
          Deconnect($conn);
  }
